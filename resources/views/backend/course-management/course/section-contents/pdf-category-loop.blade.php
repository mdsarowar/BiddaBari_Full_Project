@foreach($courseCategory->pdfStoreCategories as $index => $subCategory)
    ...
    @if(!empty($subCategory))
        @if(count($subCategory->pdfStoreCategories) > 0)
            @include('backend.course-management.course.section-contents.pdf-category-loop', ['courseCategory' => $subCategory, 'child' => $child + $child])
        @endif
    @endif
@endforeach
